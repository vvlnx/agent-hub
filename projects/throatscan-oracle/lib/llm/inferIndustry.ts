...
export async function fetchIndustryFromLLM(
  ...
  try {
    const response = await fetch(`${config.baseUrl}/chat/completions`, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          {
            role: "user",
            content: `Industry query: "${normalized}"\n\nReturn JSON matching the schema described in the system prompt.`,
          },
        ],
      }),
    ...
    const payload = parsePayload(extractJson(content), normalized);
    if (!payload) {
